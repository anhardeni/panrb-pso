...
<div class="row">
       ...
       <div class="col-sm-6">
        <?= $form->field($model, 'id_instansi')->widget(\kartik\widgets\Select2::classname(), ['data' => \yii\helpers\ArrayHelper::map(\app\models\User::find()->where(['status' => 10])->andWhere(['not', ['instansi' => null]])->orderBy('id')->asArray()->all(), 'id','instansi'),
...
<?= $form->field($model, 'pso_akhir_v')->textInput()->input('disabledTextInput',
 ...
 <div class="col-sm-6">
        <?= $form->field($model, 'flag1_analisa_usulan')->widget(\kartik\widgets\Select2::classname(), ['data' => \yii\helpers\ArrayHelper::map(\app\models\PsoAnalisa::find()->orderBy('id')->asArray()->all(),'id','analisa'),
            'options' => ['placeholder' => 'pilih hasil Analisa'],
            'pluginOptions' => [
                'allowClear' => true
            ],
        ]); ?>
...
<?= $form->field($model, 'ket_analisa')->textInput(['maxlength' => true]) ?>

<?= $form->field($model, 'id_analis')->textInput() ?>

<?= $form->field($model, 'src_filename')->textInput(['maxlength' => true]) ?>

<?= $form->field($model, 'web_filename')->textInput(['maxlength' => true]) ?>

<?= $form->field($model, 'dok_filename')->textInput(['maxlength' => true]) ?>
<!-- 
